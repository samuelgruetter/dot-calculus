...
Inductive tymode: Set := ty_precise | ty_general | ty_tight: nat -> tymode.
(* ty_tight bounds the number of unpacking *)
...
| ty_rec_intro_tight : forall n Gs G x T,
    ty_trm (ty_tight n) Gs G (trm_var (avar_f x)) (open_typ x T) ->
    ty_trm (ty_tight (S n)) Gs G (trm_var (avar_f x)) (typ_bnd T)
...
| ty_sub : forall m Gs G t T U,
    ...
    subtyp m Gs G T U ->
    ty_trm m Gs G t U
| ty_monotone_tight : forall n1 n2 Gs G t T,
    ty_trm (ty_tight n1) Gs G t T -> n1 <= n2 ->
    ty_trm (ty_tight n2) Gs G t T
...
| subtyp_sel2_s: forall m mt Gs G x A S T, m <> ty_precise ->
    ((m = ty_general /\ mt = ty_general) \/ (m <> ty_general /\ mt = ty_precise)) ->
...
Proof.
  ...
  + intros. subst.
    ...
    destruct r.
    eapply subtyp_sel2_c. assumption. eassumption. eassumption.
    destruct H2 as [G3' [Heq r]].
    eapply subtyp_sel2_c. assumption. eapply r. eapply H. eapply Heq.
  ...
  + intros. subst.
    ...
    apply weaken_restricted_by with (G2:=G2) in r.
    destruct r.
    eapply subtyp_sel1_c. assumption. eassumption. eassumption.
    destruct H2 as [G3' [Heq r]].
    eapply subtyp_sel1_c. assumption. eapply r. eapply H. eapply Heq.
    rewrite Heq in H0. eapply ok_restricted_by; eauto.
    eauto.
  + intros. subst.
...
Proof.
  ...
  induction Hsub; try solve [subst; false].
...
Lemma tight_to_general:
  (forall m Gs G t T,
     ty_trm m Gs G t T -> forall n,
     m = ty_tight n ->
     ty_trm ty_general Gs G t T) /\
  (forall m Gs G S U,
     subtyp m Gs G S U -> forall n,
     m = ty_tight n ->
     subtyp ty_general Gs G S U).
Proof.
  apply ts_mutind; intros; subst; eauto;
  try solve [econstructor; eauto; try discriminate; try reflexivity].
...
/\ (forall m Gs G S U, subtyp m Gs G S U -> forall G',
    ...
    subtyp m Gs G' S U).
Proof.
  apply rules_mutind; intros; eauto 4; try solve [false].
  - (* ty_var *)
    subst.
...
Lemma precise_fresh_ty_trm: forall Gs x T,
  ty_trm ty_precise Gs empty (trm_var (avar_s x)) T -> forall y,
    ...
    y \notin fv_typ T.
Proof.
  introv Hty. dependent induction Hty; intros; eauto; try solve [subst; false].
...
Proof.
  ...
  - assert (ty_precise <> ty_general) as Neqm by discriminate.
    specialize (H0 Neqm). destruct H0. inversion H0.
Qed.
...
Proof.
  intros y S. apply rules_mutind; intros; subst G; try subst m; try solve [false].
  - (* ty_var_s *)
    simpl.
  ...
  - (* subtyp_top *)
    apply subtyp_top. discriminate.
  - (* subtyp_bot *)
    apply subtyp_bot. discriminate.
  - (* subtyp_refl *)
    apply subtyp_refl. discriminate.
  - (* subtyp_trans *)
    eapply subtyp_trans; eauto 4.
  ...
  - (* subtyp_and2 *)
    eapply subtyp_and2; eauto 4.
  - (* subtyp_fld *)
    eapply subtyp_fld; eauto.
  - (* subtyp_typ *)
    eapply subtyp_typ; eauto 4.
  - (* subtyp_sel2_c *)
    simpl.
    case_if.
    + assert (G' = x0 ~ S) as B. {
...
Lemma subst_tight_subtyp: forall n nt Gs s x x0 v T1 T2,
  ...
  subtyp (ty_tight nt) Gs (empty & x0 ~ typ_bnd T1) (open_typ (in_ctx x0) T1) (open_typ (in_ctx x0) T2) ->
...
Proof.
  ...
  - (* All-<:-All *)
    ...
    rewrite C. eauto.
  - (* Rec-<:-Rec *)
    inversion LE. subst. inversion HT0; subst.
    assert (record_type (open_typ (in_sto x) T)) as B. {
      eapply record_type_new; eassumption.
    }
    rewrite H7 in B. destruct B as [? B]. inversion B.
  ...
  - (* Rec-<:-Rec *)
    ...
    rewrite H7 in B. destruct B as [? B]. inversion B.
    pick_fresh z.
    eapply pt_bnd. eapply IHm. omega. eassumption. assumption.
    eassumption.
    eapply subst_tight_subtyp. eassumption. eassumption. eassumption.
    eapply (H0 z). eauto. reflexivity.
Qed.
...
Lemma possible_types_completeness_tight: forall Gs s x T,
  ...
  exists v, binds x v s /\ exists n, possible_types n Gs x v T.
Proof.
  ...
  - assert (exists v, binds x v s /\ ty_trm ty_precise Gs empty (trm_val v) T) as A. {
    ...
    exists v. split. apply Bis.
    exists 1. eapply possible_types_completeness_for_values; eauto.
  - specialize (IHty_trm x Hwf).
    destruct IHty_trm as [v [Bis Hp]].
    eauto. eauto.
    exists v. split. assumption.
    destruct Hp as [n1 Hp].
    exists (0+n1+1). eapply pt_bnd. eapply Hp. reflexivity.
  - specialize (IHty_trm x Hwf).
  ...
  - specialize (IHty_trm1 x Hwf). destruct IHty_trm1 as [v [Bis1 Hp1]]; eauto.
    ...
    destruct Hp1 as [n1 Hp1]. destruct Hp2 as [n2 Hp2].
    exists (0+max n1 n2). apply pt_and; assumption.
  - specialize (IHty_trm x Hwf). destruct IHty_trm as [v [Bis Hp]]; eauto.
    exists v. split. apply Bis.
    destruct Hp as [n1 Hp].
    exists n1. eapply possible_types_closure_tight; eauto.
Qed.
...
Proof.
  ...
  destruct Hpt as [n1 Hpt].
  inversion Hpt; subst; repeat eexists; eauto.
Qed.

Lemma possible_types_closure: forall n Gs s x v S T,
  wf_sto Gs s ->
  binds x v s ->
  possible_types n Gs x v S ->
  subtyp ty_general Gs empty S T ->
  possible_types n Gs x v T.
Proof.
...
Lemma possible_types_lemma: forall Gs s x v T,
  ...
  binds x v s ->
  ty_trm ty_general Gs empty (trm_var (avar_s x)) T ->
  exists n, possible_types n Gs x v T.
Proof.
  introv Hwf Bis Hty.
...
Proof.
  ...
  lets Hp: (possible_types_lemma Hwf Bis Hty). destruct Hp as [n1 Hp].
  inversion Hp; subst.
  ...
  - pick_fresh y. exists (dom Gs \u L). exists S0. exists t.
    ...
    eapply H1; eauto.
    apply subenv_last_only. apply H6.
