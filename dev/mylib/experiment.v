...
Notation "s1 == s2" :=
  (if (Peano_dec.eq_nat_dec s1 s2) then true else false) (at level 70, no associativity).
...
match ds with
| defs_nil => None
| defs_cons ds' d => if beq_label (label_of_def d) l then Some d else get_def l ds'
end.
...
Ltac prove_binds := solve [
  unfold binds; unfold EnvOps.get;
...
Definition E: typ_label := 0.
Definition Stream: typ_label := 1.
...
Proof.
  ...
  + destruct (Peano_dec.eq_nat_dec t t0) as [Eq | Ne]; try discriminate; subst; reflexivity.
  + destruct (Peano_dec.eq_nat_dec m m0) as [Eq | Ne]; try discriminate; subst; reflexivity.
Qed.
...
Proof.
  ...
  + destruct (Peano_dec.eq_nat_dec t t) as [Eq | Ne]; try discriminate; auto.
...
Definition avar_eq_dec(a1 a2: avar): {a1 = a2} + {a1 <> a2}.
  decide equality.
...
(*
Eval cbv in (if eq_typ_dec (typ_rcd (dec_typ "L" typ_bot typ_top))
                           (typ_rcd (dec_typ "L2" typ_bot typ_top))
             then "same" else "different").
*)

Definition in_typ_list(T: typ)(l: list typ): bool :=
  if List.in_dec eq_typ_dec T l then true else false.
...
Proof.
  ...
  destruct (in_typ_list T A) eqn: EqI.
...
Definition TGlob := typ_and
 (typ_rcd (dec_typ E typ_top typ_top))
 (typ_rcd (dec_typ Stream TStream TStream)).

Definition env1: ctx := (glob ~ TGlob).
