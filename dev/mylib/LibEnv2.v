...
Definition single : var -> A -> env A := fun x v => (x,v)::nil.

Definition concat : env A -> env A -> env A := fun E F => (F ++ E)%list.
...
Fixpoint get (k : var) (E : env A) {struct E} : option A :=
  match E with
  | nil => None
  | (x,v) :: E' => if (Peano_dec.eq_nat_dec k x) then Some v else get k E'
  end.
...
Definition concat (E F : env A) := (F ++ E)%list.
Lemma concat_def : 
  concat = fun E F => (F ++ E)%list.
Proof. reflexivity. Qed.
...
Fixpoint get_impl (k : var) (E : env A) {struct E} : option A :=
  match E with
  | nil => None
  | (x,v) :: E' => if (Peano_dec.eq_nat_dec k x) then Some v else get_impl k E'
  end.
Definition get : var -> env A -> option A := get_impl.
...
Lemma get_single : forall k x v,
  get k (x ~ v) = if (Peano_dec.eq_nat_dec k x)
                    then Some v 
                    else None.
Proof. intros. rew_env_defs. auto. Qed.
...
Proof. 
  ...
  rew_app. rewrite fold_right_cons. 
Admitted.
(*rewrite IHF.
...
Lemma concat_empty_l : forall E,
  empty & E = E.
Proof. intros. rew_env_defs. rew_app~. Admitted.
...
Proof. introv H. rew_env_defs. Admitted. (*forwards*: nil_eq_app_inv H. Qed.*)
...
Lemma empty_middle_inv : forall E F x v,
  empty = E & x ~ v & F -> False.
Proof. introv H. rew_env_defs. Admitted. (*forwards* [_ ?]: nil_eq_app_inv H. false. Qed.*)
...
Proof. (* beautify *)
  unfold binds.
  introv H. rew_env_defs. unfolds get. induction E as [|[y v'] E'].
  false.
...
Proof.
  introv H. induction E2 using env_ind.
  false. Admitted. (*applys* binds_empty_inv.
...
Lemma binds_not_middle_inv : forall x v E1 E2 E3,
  ...
  \/ (x # E3 /\ binds x v E1).
Proof.
  introv H F. Admitted. (*lets [?|[? M]]: (binds_concat_inv H).
    left~.
