...
Inductive typ: Set :=
  | typ_top : typ
  | typ_bot : typ
  | typ_bind: decs -> typ (* { z => decs } (nameless self ref) *)
...
Inductive trm: Set :=
  | trm_var : avar -> trm
  | trm_new : defs -> trm (* with nameless self ref *)
...
(** ** Locally closed
Definition lc_avar(a: avar) := forall k z, open_rec_avar k z a = a.
Definition lc_pth(p: pth) := forall k z, open_rec_pth k z p = p.
Definition lc_typ(t: typ) := forall k z, open_rec_typ k z t = t.
Definition lc_dec(d: dec) := forall k z, open_rec_dec k z d = d.
Definition lc_decs(ds: decs) := forall k z, open_rec_decs k z ds = ds.
Definition lc_trm(t: trm) := forall k z, open_rec_trm k z t = t.
Definition lc_def(d: def) := forall k z, open_rec_def k z d = d.
Definition lc_defs(ds: defs) := forall k z, open_rec_defs k z ds = ds.
*)
...
Inductive red: trm -> sto -> trm -> sto -> Prop :=
  ...
  | red_call: forall s x y m T U ds body,
      binds x ds s ->
      defs_has (open_defs x ds) (def_mtd m T U body) ->
      red (trm_call (trm_var (avar_f x)) m (trm_var (avar_f y))) s
          (open_trm y body) s
  | red_sel: forall s x y l ds,
      binds x ds s ->
      defs_has (open_defs x ds) (def_fld l y) ->
...
Inductive wf_typ: pmode -> dmode -> ctx -> typ -> Prop :=
  ...
  | wf_bind_deep: forall L m G Ds,
      (forall z, z \notin L -> wf_decs m (G & z ~ typ_bind Ds) (open_decs z Ds)) ->
...
with pth_has: pmode -> ctx -> pth -> dec -> Prop :=
  | pth_has_rule: forall m G x D T Ds,
      pth_ty m G (pth_var (avar_f x)) T ->
      exp m G T Ds ->
      bdecs_has Ds D ->
      pth_has m G (pth_var (avar_f x)) (open_dec x D)
...
with trm_has: ctx -> trm -> dec -> Prop :=
  | has_trm: forall G t T Ds D,
      ...
      exp ip G T Ds ->
      bdecs_has Ds D ->
      (* wf_dec also checks that D doesn't contain the free var of expansion *)
      wf_dec ip G D ->
      trm_has G t D
  | has_var: forall G v T Ds D,
      ...
      exp ip G T Ds ->
      bdecs_has Ds D ->
      wf_dec ip G (open_dec v D) ->
      trm_has G (trm_var (avar_f v)) (open_dec v D)
  ...
  | subtyp_bind: forall L m G Ds1 Ds2 n,
      (forall z, z \notin L -> 
         subdecs m (G & z ~ typ_bind Ds1)
                   (open_decs z Ds1) 
                   (open_decs z Ds2) n) ->  (*
      The above hypothesis allows us to extract wf-ness of (open_decs z Ds1) and
      (open_decs z Ds2), for both with (typ_bind Ds1) added to env.
      For Ds2, that's not what we need (want to prove wf-ness of (typ_bind Ds2),
      so we check it again with the env we want: *)
      (forall z, z \notin L -> 
         wf_decs m (G & z ~ typ_bind Ds2) (open_decs z Ds2)) ->
      subtyp m G (typ_bind Ds1) (typ_bind Ds2) (S n)
...
Inductive wf_sto: sto -> ctx -> Prop :=
  ...
  | wf_sto_push: forall L s G x ds Ds,
      wf_sto s G ->
      x # s ->
      x # G ->
      (forall z, z \notin L \/ z = x ->
         ty_defs (G & z ~ typ_bind Ds) (open_defs z ds) (open_decs z Ds)) ->
      cbounds_decs Ds ->
      (* needed because ty_defs_regular only gives imprecise wf_decs: *)
      (forall z, z \notin L \/ z = x ->
...
(*
  | wf_sto_push: forall s G x ds Ds,
      ...
      cbounds_decs Ds ->
      (* needed because ty_defs_regular only gives imprecise wf_decs: *)
      wf_decs pr (G & x ~ typ_bind Ds) (open_decs x Ds) ->
      wf_sto (s & x ~ ds) (G & x ~ typ_bind Ds).
*)
...
Lemma typing_regular:
   (forall G t D, trm_has G t D ->
      wf_dec ip G D)
(* no need to be so complicated: whatever D it has, D is wf
        (exists x, t = trm_var (avar_f x) /\ wf_dec ip G (open_dec x D))
     \/ ((forall x, open_dec x D = D) /\ wf_dec ip G D)) *)
...
/\ (forall G ds Ds, ty_defs G ds Ds ->
      wf_decs ip G Ds).
Proof.
  apply has_sub_ty_mutind; try solve [intros; repeat split; subst;
  repeat match goal with
  | H: _ /\ _ |- _ => destruct H
  | H: wf_dec _ _ (dec_typ _ _ _) |- _ => inversions H
  | H: wf_dec _ _ (dec_fld _ _  ) |- _ => inversions H
  ...
  eauto].
  (* case subtyp_bind *)
  introv Sds IHSds IHWf. split.
  + apply_fresh wf_bind_deep as z. assert (zL: z \notin L) by auto.
    auto_specialize. auto_star.
  + apply_fresh wf_bind_deep as z. assert (zL: z \notin L) by auto.
    auto_specialize. auto_star.
Qed.
...
Proof.
  apply mutind3; try solve [intros; [constructor*
    ...
    || apply* subtyp_sel_r]].
  + (* case subtyp_bind *)
    introv Sds IHSds Wf Hle. rename n into n1. destruct n2 as [|n2]; [omega | idtac].
...
Proof.
  ...
  + unfold binds in *. rewrite get_push in *.
    ...
    - inversions H4. inversions H5. exists Ds. split. reflexivity.
      exists G (@empty typ). rewrite concat_empty_r. repeat split; auto.
...
Lemma label_of_dec_open: forall z D,
  label_of_dec (open_dec z D) = label_of_dec D.
Proof.
...
Lemma decs_has_open: forall Ds D z,
  decs_has Ds D -> decs_has (open_decs z Ds) (open_dec z D).
Proof.
  introv Has. induction Ds.
  + inversion Has.
  + unfold open_decs, open_rec_decs. fold open_rec_decs. fold open_rec_dec.
    inversions Has.
    - apply decs_has_hit. rewrite label_of_dec_open. apply (decs_hasnt_open z H2).
    - apply decs_has_skip.
      * apply (IHDs H1).
      * do 2 rewrite label_of_dec_open. assumption.
Qed.

(*
 TODO does not hold because
   [(open_dec z D1) = (open_dec z D2)] does not imply [D1 = D2]. *)
Axiom decs_has_close_admitted: forall Ds D z,
  decs_has (open_decs z Ds) (open_dec z D) -> decs_has Ds D.
...
Proof.
  ...
  + inversions H2. inversions H4. inversions p.
    lets Eq: (binds_func H6 H4). subst.
    specialize (H eq_refl Ds0 H5). subst Ds0.
    rewrite (bdecs_has_unique b H8).
    - reflexivity.
    - unfold label_of_dec in *.
      destruct D; destruct D0; simpl in *; (assumption || discriminate).
Qed.
...
Proof.
  ...
  + (* case wf_bot *) eauto.
  + (* case wf_bind_deep *)
    introv Sds IHSds Eq Ok. subst. apply_fresh wf_bind_deep as z.
    assert (zL: z \notin L) by auto.
    specialize (IHSds z zL G1 G2 (G3 & z ~ typ_bind Ds)).
    repeat rewrite concat_assoc in IHSds. apply* IHSds.
  ...
  + (* case subtyp_bind *)
    introv Sds IHSds Wf2 IHWf2 Eq Ok. subst.
    apply_fresh subtyp_bind as z; assert (zL: z \notin L) by auto.
    - specialize (IHSds z zL G1 G2 (G3 & z ~ typ_bind Ds1)).
      repeat rewrite concat_assoc in IHSds. apply* IHSds.
    - specialize (IHWf2 z zL G1 G2 (G3 & z ~ typ_bind Ds2)).
      repeat rewrite concat_assoc in IHWf2. apply* IHWf2.
  ...
  + (* case ty_call *) eauto.
  + (* case ty_new *)
    introv Tyds IH Cb Eq Ok. subst. apply_fresh ty_new as z; try assumption.
    assert (zL: z \notin L) by auto.
    specialize (IH z zL G1 G2 (G3 & z ~ typ_bind Ds)).
...
Proof.
  apply wf_mutind; try solve [intros; subst; try discriminate; simpl;
  try match goal with
  | H: pth_has _ empty _ _                 |- _ => false (has_empty H)
  | H: pth_ty empty (pth_var (avar_f _)) _ |- _ => false (ty_var_empty H)
  end;
  repeat match goal with
  ...
  (* case wf_bind_deep *)
  introv WfDs IH Eq1 Eq2. subst. clear IH (* useless *).
  pick_fresh z. assert (zL: z \notin L) by auto. specialize (IH z zL).
Qed.
*)

Lemma fv_decs_open: forall z Ds,
  fv_decs (open_decs z Ds) = (fv_decs Ds) \u \{ z }.
Admitted.
...
Proof.
  ...
  auto_star]).
  - unfold subset in *. intros x xIn.
    pick_fresh z. assert (zL: z \notin L) by auto. specialize (H z zL).
    rewrite (fv_decs_open z Ds) in H. rewrite dom_push in H.
    specialize (H x).
    do 2 rewrite in_union in H. specialize (H (or_introl xIn)).
    destruct H as [H | H].
    * assert (zx: z \notin \{ x }) by auto. apply notin_singleton in zx.
      rewrite in_singleton in H. rewrite H in zx. false* zx.
    * exact H.
  - inversions H0. apply* H.
...
(*
Lemma subst_avar_preserves_lc: forall x y, has_trm
  forall p, lc_avar p -> lc_pth (subst_avar x y p).
Proof.
...
Lemma subst_lc: forall x y,
   (forall T , lc_typ  T  -> lc_typ  (subst_typ  x y T ))
/\ (forall D , lc_dec  D  -> lc_dec  (subst_dec  x y D ))
/\ (forall Ds, lc_decs Ds -> lc_decs (subst_decs x y Ds)).
Proof.
  intros.
  apply typ_mutind; unfold lc_typ, lc_dec, lc_decs in *; intros; simpl; try reflexivity;
  ...
  (* case typ_bind *)
  introv IH Clo.
  assert (C: forall z : var, open_decs z d = d). {
    intro z. specialize (Clo z).
    unfold open_typ, open_rec_typ in Clo. fold open_rec_decs in Clo.
    inversion Clo. clear Clo.
    rewrite H0. apply H0.
  IH.
Qed.
...
Proof.
  intros.
  apply typ_mutind; try solve [intros; simpl; try reflexivity;
  ((unfold open_typ, open_rec_typ; fold open_rec_decs) ||
   (unfold open_dec, open_rec_dec; fold open_rec_typ) ||
   (unfold open_decs, open_rec_decs; fold open_rec_decs; fold open_rec_dec));
  f_equal;
...
Proof.
  ...
  + (* case wf_bind_deep *)
    introv WfDs IH Eq Tyy WfG. subst.
    apply_fresh wf_bind_deep as z. fold subst_decs.
    assert (zL: z \notin L) by auto. specialize (IH z zL G1 (G2 & z ~ typ_bind Ds) x).
    repeat rewrite concat_assoc in IH. specialize (IH eq_refl Tyy).
    rewrite subst_ctx_push in IH. simpl in IH.
    repeat rewrite concat_assoc in IH.
    lets P: (@subst_open_commute_decs x y z Ds).
    unfold subst_fvar in P. assert (x <> z) by auto. case_if.
    rewrite P in IH. apply IH.
    apply (wf_ctx_push WfG (wf_bind_deep WfDs)). auto.
  + (* case wf_bind_shallow *)
  ...
  + (* case pth_has_rule *)
    ...
    rename x0 into z.
    unfold subst_pth, subst_avar in *.
    assert (Eq: (If x = z then avar_f y else avar_f x)
              = (avar_f (If z = x then y else x))) by (case_if; case_if; reflexivity).
    rewrite Eq in *.
    rewrite subst_open_commute_dec.
    unfold subst_fvar.
    apply (pth_has_rule IHTy IHExp (subst_bdecs_has _ _ DsHas)).
  + (* case pth_ty_var *)
  ...
  + (* case pth_ty_sbsm *)
    ...
    - apply* IHSt.
  + (* case has_trm *)
    introv Ty IHTy Exp IHExp DsHas WfD IHWf Eq Tyy Wf. subst.
    specialize (IHTy _ _ _ eq_refl Tyy Wf).
  ...
  + (* case has_var *)
    ...
    - (* case z = x *)
      ...
      * lets P: (@subst_open_commute_dec x y x D). unfold subst_fvar in P. case_if.
        rewrite <- P.
        apply* IHWf.
    - (* case z <> x *)
      rename v into z.
      ...
      * apply* IHExp.
      * apply (subst_bdecs_has x y DsHas).
      * lets P: (@subst_open_commute_dec x y z D). unfold subst_fvar in P. case_if.
        rewrite <- P.
        apply* IHWf.
  + (* case subtyp_refl *)
  ...
  + (* case subtyp_bind *)
    intros L m G Ds1 Ds2 n Sds IHSds Wf2 IHWf2 G1 G2 x EqG Bi WfG. subst.
    apply_fresh subtyp_bind as z; fold subst_decs; assert (zL: z \notin L) by auto.
    - specialize (IHSds z zL G1 (G2 & z ~ typ_bind Ds1) x).
      rewrite concat_assoc in IHSds.
      specialize (IHSds eq_refl Bi).
      unfold subst_ctx in IHSds. rewrite map_push in IHSds. simpl in IHSds.
      rewrite concat_assoc in IHSds.
      rewrite (subst_open_commute_decs x y z Ds1) in IHSds.
      rewrite (subst_open_commute_decs x y z Ds2) in IHSds.
      unfold subst_fvar in IHSds.
      assert (x <> z) by auto. case_if.
      unfold subst_ctx. apply IHSds.
      apply (wf_ctx_push WfG).
      * apply_fresh wf_bind_deep as v.
        assert (vL: v \notin L) by auto. specialize (Sds v vL).
        destruct (subdecs_regular Sds) as [Wf1 _]. exact Wf1.
      * auto.
    - specialize (IHWf2 z zL G1 (G2 & z ~ typ_bind Ds2) x).
      repeat rewrite concat_assoc in IHWf2.
      specialize (IHWf2 eq_refl Bi).
      rewrite subst_ctx_push in IHWf2.
      repeat rewrite concat_assoc in IHWf2. simpl in IHWf2.
      lets P: (@subst_open_commute_decs x y z Ds2). unfold subst_fvar in P.
      assert (x <> z) by auto. case_if.
      rewrite <- P.
      apply IHWf2.
      apply (wf_ctx_push WfG).
      * apply_fresh wf_bind_deep as v.
        assert (vL: v \notin L) by auto. apply (Wf2 v vL).
      * auto.
  + (* case subtyp_sel_l *)
...
*)
  ...
  + (* case ty_new *)
    intros L G ds Ds Tyds IHTyds Cb G1 G2 x Eq Bi Wf. subst G.
    apply_fresh ty_new as z.
    - fold subst_defs. fold subst_decs.
      ...
      apply IHTyds.
      apply (wf_ctx_push Wf).
      * apply_fresh wf_bind_deep as v.
        assert (vL: v \notin L) by auto. specialize (Tyds v vL).
...
Lemma invert_simple_ctx: forall G,
  simple_ctx G ->
  ok G /\
  forall x T, binds x T G -> exists L Ds,
     T = typ_bind Ds /\
    (forall z, z \notin L \/ z = x ->
       gbounds_decs (G & z ~ typ_bind Ds) (open_decs z Ds)) /\
    (forall z, z \notin L \/ z = x ->
...
Admitted. (*
  - assert (Ok': ok (G & x ~ typ_bind Ds)) by auto_star.
    apply (conj Ok').
    introv Bi. destruct IHSc as [Ok F].
    apply binds_push_inv in Bi. destruct Bi as [[Eq1 Eq2] | [Ne Bi]].
    * subst. exists vars_empty Ds. apply (conj eq_refl).
      split.
...
Lemma invert_pth_has_pr_in_simple_ctx: forall G x D,
  simple_ctx G ->
  ...
  exists Ds D',
    ...
    D = open_dec x D'.
Proof.
  introv Sc Has. inversions Has. inversions H0.
  lets P: (invert_simple_ctx Sc). destruct P as [_ P].
  specialize (P x T H5). destruct P as [L [Ds0 [Eq _]]].
  subst. inversions H1. inversions H4. eauto.
Qed.

Axiom sc_admit: forall G, simple_ctx G.
...
Lemma binds_middle_change: forall (A: Type) y x (T S1 S2: A) G1 G2,
  y <> x ->
  binds y T (G1 & x ~ S1 & G2) ->
  binds y T (G1 & x ~ S2 & G2).
Proof.
Admitted.

(* Needs no induction because we're in a typ_bind-only env, so the exp inside the
   pth_has is exp_bind, which is trivial *)
Lemma narrow_pth_has: forall G1 G2 x DsA DsB n0 v D2,
  simple_ctx (G1 & x ~ typ_bind DsA & G2) ->
  subdecs pr (G1 & x ~ typ_bind DsA) (open_decs x DsA) (open_decs x DsB) n0 ->
  pth_has pr (G1 & x ~ typ_bind DsB & G2) (pth_var (avar_f v)) D2 ->
  exists n1 D1,
    pth_has pr (G1 & x ~ typ_bind DsA & G2) (pth_var (avar_f v)) D1 /\ 
    subdec  pr (G1 & x ~ typ_bind DsA & G2) D1 D2 n1.
Proof.
  introv ScA SdsAB Has.
  destruct (invert_simple_ctx ScA) as [OkA _].
  lets ScB: (sc_admit (G1 & x ~ typ_bind DsB & G2)).
  destruct (invert_simple_ctx ScB) as [OkB _].
  lets WfG: (simple_ctx_to_wf_ctx ScA).
  lets P: (middle_notin WfG). destruct P as [xG1 [_ [_ xG2]]].
  apply (invert_pth_has_pr_in_simple_ctx ScB) in Has.
  destruct Has as [Ds [D2' [Bi [DsHas Eq0]]]].
  destruct (classicT (v = x)) as [Eq | Ne].
  - (* case v = x *)
    subst. lets Eq: (binds_middle_eq_inv Bi OkB). subst.
    inversions Eq.
    destruct (subdecs_regular SdsAB) as [WfA _].
    apply (decs_has_open x) in DsHas.
    lets P: (decs_has_preserves_sub DsHas SdsAB).
    destruct P as [D1 [DsAHas Sd]].
    unfold open_dec, open_rec_dec in Sd. fold open_rec_typ open_rec_dec in Sd.
    assert (E: exists D1', D1 = open_dec x D1') by admit.
    destruct E as [D1' Eq].
    exists n0 D1. subst.
    split.
    + refine (pth_has_rule (pth_ty_var _ (binds_middle_eq _ _ xG2))
                             (exp_bind _ _ _)
                             (bdecs_has_decs _)).
      apply (decs_has_close_admitted DsA _ x DsAHas).
    + apply (weaken_subdec_end OkA Sd).
  - (* case v <> x *)
    exists n0 D2. subst.
    split.
    + refine (pth_has_rule (pth_ty_var _ (binds_middle_change _ Ne Bi))
                             (exp_bind _ _ _)
                             (bdecs_has_decs DsHas)).
    + apply subdec_refl. admit. (* TODO wf-ness follows from Bi and ScB *)
Qed.
...
/\ (forall m G t D2, pth_has m G t D2 -> forall G1 G2 x DsA DsB n0,
    ...
    exists n1 D1,
      ...
      subdec pr  (G1 & x ~ typ_bind DsA & G2) D1 D2 n1) *)
/\ (forall m G T1 T2 n, subtyp m G T1 T2 n -> forall G1 G2 x DsA DsB n0,
    m = pr ->
    G = G1 & x ~ (typ_bind DsB) & G2 ->
    simple_ctx (G1 & x ~ typ_bind DsA & G2) ->
    subdecs pr (G1 & x ~ (typ_bind DsA)) (open_decs x DsA) (open_decs x DsB) n0 ->
    subtyp pr (G1 & x ~ (typ_bind DsA) & G2) T1 T2 n)
/\ (forall m G D1 D2 n, subdec m G D1 D2 n -> forall G1 G2 x DsA DsB n0,
    m = pr ->
    G = G1 & x ~ (typ_bind DsB) & G2 ->
    simple_ctx (G1 & x ~ typ_bind DsA & G2) ->
    subdecs pr (G1 & x ~ (typ_bind DsA)) (open_decs x DsA) (open_decs x DsB) n0 ->
    subdec pr (G1 & x ~ (typ_bind DsA) & G2) D1 D2 n)
/\ (forall m G Ds1 Ds2 n, subdecs m G Ds1 Ds2 n -> forall G1 G2 x DsA DsB n0,
    m = pr ->
    G = G1 & x ~ (typ_bind DsB) & G2 ->
    simple_ctx (G1 & x ~ typ_bind DsA & G2) ->
    subdecs pr (G1 & x ~ (typ_bind DsA)) (open_decs x DsA) (open_decs x DsB) n0 ->
...
Proof.
  ...
  + (* case wf_bot *) eauto.
  + (* case wf_bind_deep *)
    introv WfDs IHWf Eq1 Eq2 Sc Sds. subst.
    apply_fresh wf_bind_deep as z. assert (zL: z \notin L) by auto.
  ...
  + (* case wf_bind_shallow *) eauto.
  + (* case wf_sel1 *)
    introv Has2 WfLo IHLo WfHi IHHi Eq1 Eq2 ScA SdsAB. subst. rename x into v, x0 into x.
    specialize (IHLo _ _ _ _ _ _ eq_refl eq_refl ScA SdsAB).
    specialize (IHHi _ _ _ _ _ _ eq_refl eq_refl ScA SdsAB).
    (* if x = v, these IHs are useless, because x doesn't have L:Lo..Hi any more *)
    lets P: (narrow_pth_has ScA SdsAB Has2).
    destruct P as [n1 [D1 [Has1 Sd]]].
    apply invert_subdec_typ_sync_left in Sd.
    destruct Sd as [Lo1 [Hi1 [Eq [StLo StHi]]]]. subst.
    destruct (invert_simple_ctx ScA) as [OkA _].
    apply (wf_sel1 Has1).
    - apply wf_deep_to_any. apply (proj2 (subtyp_regular StLo)).
    - apply wf_deep_to_any. apply (proj1 (subtyp_regular StHi)).
    (* Note: We never used the IHs, because we got all wf-jugments need out of the
       simple_ctx. -> Too strong hypothesis?? *)
  (*
  + (* case wf_sel1 *)
    introv Has WfLo IHLo WfHi IHHi Eq1 Eq2 ScA SdsAB. subst. rename x into v, x0 into x.
    specialize (IHLo _ _ _ _ _ _ eq_refl eq_refl ScA SdsAB).
    specialize (IHHi _ _ _ _ _ _ eq_refl eq_refl ScA SdsAB).
    (* if x = v, these IHs are useless, because x doesn't have L:Lo..Hi any more *)
    destruct (invert_simple_ctx ScA) as [OkA _].
    ...
    lets WfG: (simple_ctx_to_wf_ctx ScA).
    lets P: (middle_notin WfG). destruct P as [xG1 [_ [_ xG2]]].
    (* instead of applying an IH on Has: *)
    apply (invert_pth_has_pr_in_simple_ctx ScB) in Has.
    destruct Has as [Ds [D [Bi [DsHas Eq]]]].
    destruct D as [L' Lo' Hi' |  | ] ; inversions Eq.
    rename L' into L, Lo' into Lo, Hi' into Hi.
    destruct (classicT (v = x)) as [Eq | Ne].
    - (* case v = x *)
      subst. lets Eq: (binds_middle_eq_inv Bi OkB). subst.
      inversions Eq.
      ...
      apply invert_subdec_typ_sync_left in Sd.
      destruct Sd as [Lo1 [Hi1 [Eq [StLo StHi]]]]. subst.
      assert (E: exists Lo1' Hi1', (dec_typ L Lo1 Hi1) = open_dec x (dec_typ L Lo1' Hi1'))
        by admit.
      destruct E as [Lo1' [Hi1' Eq]].
      apply wf_sel1 with Lo1 Hi1.
      * rewrite Eq.
        refine (pth_has_rule (pth_ty_var _ (binds_middle_eq _ _ xG2))
                             (exp_bind _ _ _)
                             (bdecs_has_decs _)).
        apply (decs_has_close_admitted DsA _ x). rewrite <- Eq. exact DsAHas.
      * apply wf_deep_to_any. apply (weaken_wf_typ_end OkA).
        apply (proj2 (subtyp_regular StLo)).
      * apply wf_deep_to_any. apply (weaken_wf_typ_end OkA).
        apply (proj1 (subtyp_regular StHi)).
    - (* case v <> x *)
      apply wf_sel1 with (open_typ v Lo) (open_typ v Hi).
      * assert (Eq: dec_typ L (open_typ v Lo) (open_typ v Hi)
                  = open_dec v (dec_typ L Lo Hi)) by reflexivity.
        rewrite Eq.
        refine (pth_has_rule (pth_ty_var _ (binds_middle_change _ Ne Bi))
                             (exp_bind _ _ _)
                             (bdecs_has_decs DsHas)).
    ...
    *)
  + (* case wf_sel2 *)
    introv Has WfU IH Eq1 Eq2 ScA SdsAB. subst. rename x into v, x0 into x.
    specialize (IH _ _ _ _ _ _ eq_refl eq_refl ScA SdsAB).
    (* if x = v, the IH is useless, because x doesn't have L:Bot..U any more *)
    destruct (invert_simple_ctx ScA) as [OkA _].
    lets ScB: (sc_admit (G1 & x ~ typ_bind DsB & G2)).
    destruct (invert_simple_ctx ScB) as [OkB _].
    lets WfG: (simple_ctx_to_wf_ctx ScA).
    ...
    destruct Has as [Ds [D [Bi [DsHas Eq]]]].
    destruct D as [L' Lo' Hi' |  | ] ; inversions Eq.
    destruct Lo'; inversions H1. (* finds Lo' = typ_bot *)
    rename L' into L, Hi' into Hi.
    destruct (classicT (v = x)) as [Eq | Ne].
    - (* case v = x *)
      subst. lets Eq: (binds_middle_eq_inv Bi OkB). subst.
      inversions Eq.
      ...
      assert (E: exists Lo1' Hi1', (dec_typ L Lo1 Hi1) = open_dec x (dec_typ L Lo1' Hi1'))
        by admit.
      destruct E as [Lo1' [Hi1' Eq]].
      apply wf_sel1 with Lo1 Hi1. (* wf_sel1 works, copy-pasted, so why not? *)
      * rewrite Eq.
        refine (pth_has_rule (pth_ty_var _ (binds_middle_eq _ _ xG2))
                             (exp_bind _ _ _)
                             (bdecs_has_decs _)).
      ...
      * apply wf_deep_to_any. apply (weaken_wf_typ_end OkA).
        apply (proj1 (subtyp_regular StHi)).
    - (* case v <> x *)
      apply wf_sel2 with (open_typ v Hi).
      * assert (Eq: dec_typ L typ_bot (open_typ v Hi)
                  = open_dec v (dec_typ L typ_bot Hi)) by reflexivity.
        rewrite Eq.
        refine (pth_has_rule (pth_ty_var _ (binds_middle_change _ Ne Bi))
                             (exp_bind _ _ _)
                             (bdecs_has_decs DsHas)).
  ...
  + (* case subtyp_bot *) eauto.
  + (* case subtyp_bind *)
    introv Sds IHSds Wf2 IHWf2 Eq1 Eq2 ScA SdsAB. subst.
    apply_fresh subtyp_bind as z; assert (zL: z \notin L) by auto.
    - specialize (IHSds z zL G1 (G2 & z ~ typ_bind Ds1) x DsA DsB n0 eq_refl).
      repeat rewrite concat_assoc in IHSds.
      refine (IHSds eq_refl _ SdsAB).
      apply sc_admit. (* <-- TODO *)
    - specialize (IHWf2 z zL G1 (G2 & z ~ typ_bind Ds2) x DsA DsB n0 eq_refl).
      repeat rewrite concat_assoc in IHWf2.
      refine (IHWf2 eq_refl _ SdsAB).
      apply sc_admit. (* <-- TODO *)
  + (* case subtyp_sel_l *)
    introv Has2 St IH Eq1 Eq2 ScA SdsAB. subst.
    specialize (IH _ _ _ _ _ _ eq_refl eq_refl ScA SdsAB).
    lets P: (narrow_pth_has ScA SdsAB Has2).
    destruct P as [n1 [D1 [Has1 Sd]]].
    apply invert_subdec_typ_sync_left in Sd.
    ...
    - apply (subtyp_sel_l Has1).
      lets P: (has_good_bounds ScA Has1). destruct P as [n2 StLoHi].
      assert (n2 = n) by admit. subst. exact StLoHi.
    - assert (n1 = n) by admit. subst. exact StHi.
  + (* case subtyp_sel_r *)
    introv Has2 St IH Eq1 Eq2 ScA SdsAB. subst.
    specialize (IH _ _ _ _ _ _ eq_refl eq_refl ScA SdsAB).
    lets P: (narrow_pth_has ScA SdsAB Has2).
    ...
    - assert (n1 = n) by admit. subst. exact StLo.
    - apply (subtyp_sel_r Has1).
      lets P: (has_good_bounds ScA Has1). destruct P as [n2 StLoHi].
      assert (n2 = n) by admit. subst. exact StLoHi.
  + (* case subtyp_trans *)
    introv St12 IH12 St23 IH23 Eq1 Eq2 ScA SdsAB. subst.
    specialize (IH12 _ _ _ _ _ _ eq_refl eq_refl ScA SdsAB).
    specialize (IH23 _ _ _ _ _ _ eq_refl eq_refl ScA SdsAB).
    apply (subtyp_trans IH12 IH23).
  + (* case subdec_typ *) eauto.
  + (* case subdec_fld *) eauto.
  + (* case subdec_mtd *) eauto.
  + (* case subdecs_empty *) eauto.
  + (* case subdecs_push *) eauto.
Qed.
...
Proof.
  introv ScA SdsAB Sds12.
  destruct narrowing as [_ [_ [_ [_ [_ N]]]]].
  specialize (N pr (G & y ~ typ_bind DsB) Ds1 Ds2 n Sds12 G empty y DsA DsB n eq_refl).
  repeat rewrite concat_empty_r in N. apply* N.
Qed.
