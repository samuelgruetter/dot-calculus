...
Proof.
  ...
  + inversions Eq. exists T (ctx_size G).
  ...
  + subst. specialize (IHTy _ eq_refl). destruct IHTy as [T' [n2 [St Bi]]].
    ...
    - apply subtyp_trans with T.
      * refine (subtyp_ctx_size _ St). apply Max.le_max_r.
      * refine (subtyp_ctx_size _ H). apply Max.le_max_l.
    - exact Bi.
Qed.
...
Proof.
  ...
  + (* case has_pr *)
    ...
    - (* case z <> x *)
      ...
      * apply (subst_decs_has x y DsHas).
  + (* case subtyp_refl *)
    introv EqG Ty Ok. subst. admit. (* TODO trivial *)
  + (* case subtyp_top *)
    intros. simpl. subst.  admit. (* TODO trivial *)
  + (* case subtyp_bot *)
    intros. simpl. subst.  admit. (* TODO trivial *)
  + (* case subtyp_bind *)
  ...
  + (* case subtyp_trans *)
    ...
    apply* subtyp_trans.
  + (* case subtyp_ctx_size *)
    intros. subst. apply* subtyp_ctx_size.
  + (* case subdec_typ *)
  ...
  + (* case subdec_mtd *)
    intros. apply* subdec_mtd.
  + (* case subdecs_empty *)
    intros. subst. apply subdecs_empty. admit. (* TODO trivial *)
  + (* case subdecs_push *)
  ...
  + (* case subdecs_refl *)
    introv Hle EqG Ty Ok. apply subdecs_refl. subst. admit. (* TODO trivial *)
  + (* case ty_var *)
...
/\ (forall m G t l D2, has m G t l D2 -> forall G1 G2 x DsA DsB,
    ...
    exists D1,
      ...
      subdec pr (G1 & x ~ (typ_bind DsA) & G2) D1 D2
                ((ctx_size (G1 & x ~ typ_bind DsA & G2)) + d))
...
Proof.
  ...
  + (* case exp_sel *)
    ...
    intros G1 G2 x DsA DsB E1 E2 Ok2 SdsAB.
    specialize (IHHas _ _ _ _ _ E1 E2 Ok2 SdsAB).
    specialize (IHExp _ _ _ _ _ E1 E2 Ok2 SdsAB).
    subst.
    ...
    (* case d = 0 *) {
    subst d. clear E.
    lets E: (exp_preserves_sub_base_case StHi12 eq_refl (Plus.plus_0_r _) Exp1 IHExp).
    ...
    lets Sds1: (E _ _ Hi1 Hi2 Ds1 Dsm _ StHi12 eq_refl Exp1 IHExp).
    ...
    assert (Eq2: ctx_size (G1 & x ~ typ_bind DsA & G2) + max (d + 1) 1
               = ctx_size (G1 & x ~ typ_bind DsA & G2 & z ~ typ_bind Dsm) + d). {
      rewrite Eq4. rewrite ctx_size_push. omega.
    }
    assert (Sds2n: subdecs pr (G1 & x ~ typ_bind DsA & G2 & z ~ typ_bind Ds1 & empty)
      ...
      rewrite <- Eq2.
      refine (P pr _ _ _ _ Sds2 _ _ _ _ _ eq_refl Eq2 _ _ _).
      - apply okadmit.
    ...
    rewrite Eq4. rewrite Plus.plus_assoc.
  ...
  + (* case has_pr *)
    ...
    - (* case v = x *)
      ...
      apply (weaken_subdec_end _ OkA) in Sd.
      refine (conj _ Sd).
